...
const createAppStore = () => {
  ...
  return {
    repository,
    store: createSurfaceGateDeskStore(surfaceGateDeskFixture, repository.save),
  };
};
...
export default function App() {
  const { repository, store } = useMemo(createAppStore, []);
  const state = useSurfaceGateDeskState(store);
  const actions = store.actions;
  const runAction = useCallback(
    (actionId: string, label: string) => () => actions.acknowledgeAction(`${label} (${actionId})`),
    [actions],
  );

  useEffect(() => {
    const loaded = repository.load();
    const storageStatus = loaded.recovered ? 'recovered' : loaded.error ? 'unavailable' : 'ready';
    actions.hydrate(loaded.snapshot, storageStatus, loaded.error);
  }, [actions, repository]);
  ...
  }), [actions, navigationActions, runAction]);
  ...
  }), [actions, navigationActions, runAction, state.preferences]);
  ...
  return (
    <div data-setfarm-root="surfacegate-desk" data-active-screen={state.activeScreen} className="min-h-screen bg-slate-50 text-slate-950">
      ...
      {state.activeScreen === 'help' ? (
        <EmptyAndErrorRecoverySurfacegateDesk actions={helpActions} />
      ) : null}
